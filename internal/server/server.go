	...
	for {
		...
		if n > 0 {
			...
			if slices.Contains(cmdBytes, '\n') {
				var response string
				response, err := s.execute(string(cmdBytes))
				if err != nil {
...
func NewServer(port string, logger *zap.Logger, readTimeout time.Duration) *Server {
	s := &Server{Port: port, Logger: logger, ReadTimeout: readTimeout, store: store.NewMemStore()}
